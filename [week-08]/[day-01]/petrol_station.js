...
let car = {
  ...
  refill: function(amount) {
    ...
    let consumpted = 0;
    if (this.petrolCapacity - this.petrolLevel >= amount) {
      consumpted = amount
    } else if (this.petrolCapacity - this.petrolLevel < amount){
      consumpted = amount + (this.petrolCapacity - this.petrolLevel)
    }
    console.log(consumpted);
    return consumpted
  }
}
